function [cost, revenue] = cost_uLink_csr_gen_reliability_large_n(num,demand,config, total_amount_served,voltage)
...
cost.solar_panel = config.pv_final_cost;
cost.battery = config.batt_final_cost;
...
%might not be as realistic with larger battery bank
...
j=1;
%if config.lifetime > config.Battery_Lifetime
...
revenue.breakeven_monthly = -payper(config.discount_rate,config.payback,cost.npv_total)/(12*num.consumers);

revenue.breakeven_monthly_connection = -payper(config.discount_rate,config.payback,cost.npv_total_connection)/(12*num.consumers);
