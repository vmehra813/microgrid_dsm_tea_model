function [reliability_total, total_amt_hours, cycles] = connect_ulink_customer_gen_sizing_kibam_load_dispatch_SAIDI(available_solar,battery,loadprof,num,batt,load)
...
addpath('D:\Claudio\Dropbox (MIT)\[uLink]\Simulation\matlab\uLink\Modeling Analysis\Generation + Reliability Model\rainflow');
...
%Initialize Matrices
total.Q_1 = zeros(length(loadprof),1); %some have an added a row bc of t+1 iteration later on
total.Q_2 = zeros(length(loadprof),1);
%battEcum = zeros(length(loadprof),1);
total.battEoutMax = zeros(length(loadprof),1);
total.battEinMax = zeros(length(loadprof),1);
total.Ebatt_max_in_lim1 = zeros(length(loadprof),1);
total.Ebatt_max_in_lim2 = zeros(length(loadprof),1);
total.Ebatt_max_in_lim3 = zeros(length(loadprof),1);
total.Ebatt_max_in_limSOCmax = zeros(length(loadprof),1);
total.EoutMax_kineticmodellimit = zeros(length(loadprof),1);
total.battSOC = zeros(length(loadprof),1);
total.Q_total = zeros(length(loadprof),1);


% critical.Q_1 = zeros(length(loadprof),1); %some have an added a row bc of t+1 iteration later on
% critical.Q_2 = zeros(length(loadprof),1);
% %battEcum = zeros(length(loadprof),1);
% critical.battEoutMax = zeros(length(loadprof),1);
% critical.battEinMax = zeros(length(loadprof),1);
% critical.Ebatt_max_in_lim1 = zeros(length(loadprof),1);
% critical.Ebatt_max_in_lim2 = zeros(length(loadprof),1);
% critical.Ebatt_max_in_lim3 = zeros(length(loadprof),1);
% critical.Ebatt_max_in_limSOCmax = zeros(length(loadprof),1);
% critical.EoutMax_kineticmodellimit = zeros(length(loadprof),1);
% critical.battSOC = zeros(length(loadprof),1);
% critical.Q_total = zeros(length(loadprof),1);


battery.initial_capacity = .5;
...
%power.in_out_batt_total = avail_solar - loadprof;
power.in_out_batt_total =  loadprof(:,end) - avail_solar; %negative is charging
...
network.total.amountserved.cumulative = zeros(length(loadprof),1);
network.total.nonserved = zeros(length(loadprof),1);
%network.total.nonserved = [];
%network.total.amountserved.cumulative(1) = 20;
network.total.solar_load_coincidence = zeros(length(loadprof),1);

network.total.rel_hour = zeros(length(loadprof),1);
network.critical.rel_hour = zeros(length(loadprof),1);


SOC.total = zeros(length(loadprof), 1);
SOC.total_spillage = zeros(length(loadprof),1);
SOC.total_cycles = zeros(1,1);

%Critical
%initialize vectors
%network.critical.reliability = zeros(1,1);
%network.critical.amountserved.cumulative = zeros(length(loadprof),1);
%network.critical.nonserved = zeros(length(loadprof),1);
%network.critical.nonserved = [];
%network.total.amountserved.cumulative(1) = 20;
%network.critical.solar_load_coincidence = zeros(length(loadprof),1);


%SOC.critical = zeros(length(loadprof), 1);
%SOC.critical_spillage = zeros(length(loadprof),1);
%SOC.critical_cycles = zeros(1,1);

%count variables
% zero = current; 1 = KIBAM; 2 = SOC
%SOC.charge_total_limit = zeros(length(loadprof),1);
%SOC.charge_critical_limit = zeros(length(loadprof),1);
%SOC.discharge_total_limit = zeros(length(loadprof),1);
%SOC.discharge_critical_limit = zeros(length(loadprof),1);
...
total.count_phone = zeros(length(loadprof),1);
total.count_fan = zeros(length(loadprof),1);
total.count_light = zeros(length(loadprof),1);
total.curtail = zeros(length(loadprof),1);
...
%for j=1:num.consumers %column is B customer    
    for i=1
        ...
        if current.in_out_batt_total(i) < 0     %charging
            ...
            % total spillage (Wh)
            SOC.total_spillage(i) = abs(avail_solar(i)-loadprof(i,end)-battery.voltage*total.Q_1(i))*ge(avail_solar(i),loadprof(i,end)+battery.voltage*total.Q_1(i));
        ...
        elseif current.in_out_batt_total(i) > 0 %discharging
            ...
            network.total.nonserved(i) = (loadprof(i,end) - battery.voltage*total.Q_1(i)-avail_solar(i))*ge(loadprof(i,end),(avail_solar(i)+battery.voltage*total.Q_1(i)));
 
        end
        
    end
    
    for i=2:length(loadprof)
        if current.in_out_batt_total(i) < 0       % charging
            ...
            SOC.total_spillage(i) = abs(avail_solar(i)-loadprof(i,end)-battery.voltage*total.battEinMax(i))*ge(avail_solar(i),loadprof(i,end)+battery.voltage*total.battEinMax(i));
            %if loadprof(i,end) > 0      
            %network.total.nonserved(i) = (loadprof(i,end) -avail_solar(i))*ge(loadprof(i,end),avail_solar(i));
            %network.total.rel_hour(i) = 1 - network.total.nonserved(i)/loadprof(i);
            %end 
        elseif current.in_out_batt_total(i) == 0
            
%             total.Q_1(i) = total.Q_1(i-1)*e_raised+(total.Q_total(i-1)*k*c-current.in_out_batt_total(i))*(1-e_raised)/k-current.in_out_batt_total(i)*c*(k*deltaT-1+e_raised)/k;
%             total.Q_2(i) = total.Q_2(i-1)*e_raised+(total.Q_total(i-1)*(1-c))*(1-e_raised)-current.in_out_batt_total(i)*(1-c)*(k*deltaT-1+e_raised)/k;
%             total.Q_total(i) = total.Q_1(i) + total.Q_2(i);
%             total.battSOC(i) = abs(total.Q_total(i)/Qmax_batt);               
            
%             
            total.Q_1(i) = total.Q_1(i-1);
            total.Q_2(i) = total.Q_2(i-1);
            total.Q_total(i) = total.Q_total(i-1);
            total.battSOC(i) = total.battSOC(i-1);
        ...
        network.total.nonserved(i) = (loadprof(i,end) - battery.voltage*total.Q_1(i))*ge(loadprof(i,end),battery.voltage*total.Q_1(i));
        network.total.rel_hour(i) = 1 - network.total.nonserved(i)/loadprof(i);
        
    end
      
network.total.reliability = 1- (sum(network.total.nonserved)/sum(loadprof(:,end))); 

network.total.rel_count = sum(network.total.rel_hour < num.total_hour_threshold);

    %j=j+1;
    
reliability_total = network.total.reliability;
total_amt_hours = network.total.rel_count;

%total_amount_served = sum(loadprof(:,end)) - sum(network.total.nonserved);
...
%             %SOC.critical_spillage(i) = abs(avail_solar(i)-loadprof(i)+battery.voltage*critical.battEinMax(i))*ge(avail_solar(i),loadprof.final.critical(i)-battery.voltage*critical.battEinMax(i));
...
%         network.critical.nonserved(i) = (loadprof.final.critical(i,end) - battery.voltage*critical.Q_1(i)-avail_solar(i))*ge(loadprof.final.critical(i,end),(avail_solar(i)+battery.voltage*critical.Q_1(i)));
% 
%         network.critical.rel_hour(i) = 1 - network.critical.nonserved(i)/loadprof.final.critical(i);
%         
%     end
%     
...
%     %j=j+1;
% 
% network.critical.rel_count = sum(network.critical.rel_hour < loadprof.critical_hour_threshold);
% 
% 
...
% 
% critical_amt_hours = network.critical.rel_count;
% 
% 
...
% figure(13);
% hold on;
% plot(loadprof(:,end)-network.total.amountserved.cumulative(:,end));
% xlabel('Hour of Year');
% ylabel('Watts');
...
% figure;
% subplot(2,1,1),scatter(1:8760,1-network.total.nonserved./loadprof(:,end),'r');
% ylim([0,1.01]);
% xlabel('Hour of Year');
% ylabel('Reliability (%)');
% set(gca,'fontsize',15);
% 
% subplot(2,1,2),plot(loadprof(:,end));
% hold on;
...
% tot_rel = 1-network.total.nonserved./loadprof(:,end);
% 
% figure;
% hist(crit_rel,100);
% 
